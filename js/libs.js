...
var widgetPrivText = {
  ...
  __url_get_proof: 'https://privtext.com'+'/api/proof',
  __url_set_encrypted: 'https://privtext.com'+'/api/note',
  ...
  run:  function(mode){
    ...
    if(self.__button_selector_start){
      $(self.__button_selector_start).click(function(){ self.start(); return false; });
    }
  },
          ...
          setTimeout(
            function(){
              if(self.__mode == self.MODE_DECODE){
                ...
                var noteid = self._global_message['data']['note'].split('/').slice(-1).pop().replace('.html', '').split('#')[0];
  ...
  __read_note: function(s_key){
    ...
    if(!s_key){
      ...
      var d = self._global_message['data']['note'].split('/').slice(-1).pop().replace('.html', '').split('#');
      s_key = d.length > 1 ? d.pop() : '';
        ...
        function(data) { // success action
          ...
          if(data.ajax_status == 'ok'){
            ...
            if(data.error_type == 'invalid-proof-password'){
              ...
              $('.readedNoteID').html(self.___noteid)
              $('.empty-note').show();
              $('.presented-note').hide();
              $('.note-delete-message').hide();
              $('.note-delete-message-now').show();
  ...
  __create_note: function(){
    ...
    var ajax_handler = self.__create_ajax_handler(
      ...
      function(data){ // success action
        if(data.ajax_status == 'ok'){
          var note_url = 'https://privtext.com'+data.url + ((!_PAGE_OPENED_LEFT_SETTINGS_BOX || (_PAGE_OPENED_LEFT_SETTINGS_BOX && !$('#encpasswd').val().length)) ? '#'+s_key : '');
